// /src/components/CreatingPostForm.js
...
const PostPostItemBlock = ({ socialGroupId, handlePostAdded }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await postPost( socialGroupId, content, filesList, visibility);
      if (response.success) {
        console.log('Bình luận đã gửi thành công:', response.data);
        handlepostAdded(response.data)
        setContent('');
  ...
  return (
    <div className="flex space-x-2 flex-row w-full ">
      ...
      {/* Nội dung của thêm post */}
      <div className="w-full relative">
        <form onSubmit={handleSubmit}>
          <div className="flex">
            <div className="flex-1">
              <div className="py-2 px-2 mb-2 bg-white rounded-lg rounded-t-lg border border-gray-200   
 ...
 focus:outline-none dark:text-white dark:placeholder-gray-400 dark:bg-gray-800"
              ...
              <button type="submit"
                className="p-0 inline-flex items-center py-2.5 px-4 text-xs font-medium text-center text-white   
...
export default PostPostItemBlock;
